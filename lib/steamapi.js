require("dotenv").config({ path: "../.env" });
const API_KEY = process.env.STEAM_API_KEY;
const {pRateLimit} = require("p-ratelimit");

const priceLimit = pRateLimit({
  interval: 1000, 
  rate: 30,
  concurrency: 1
});
const posterLimit = pRateLimit({
  interval: 1000, 
  rate: 30,
  concurrency: 2
});
    const gameLimit = pRateLimit({
      interval: 1000,
      rate: 200,
      concurrency: 15,
    });
// const gamePriceCache = new Map(); // cache for lowest game prices

async function getPlayerSummaries(steamids) {
  const idsString = steamids.join(",");
  const response = await fetch(
    `https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=${API_KEY}&steamids=${idsString}`
  );
  const data = await response.json();
  return data.response.players || [];
}

async function getfriendIDs(steamid) {
  const response = await fetch(
    `https://api.steampowered.com/ISteamUser/GetFriendList/v0001/?key=${API_KEY}&steamid=${steamid}&relationship=friend`
  );
  const data = await response.json();
  const friendIDs = data.friendslist
    ? data.friendslist.friends.map((friend) => friend.steamid)
    : [];
  const friendsSince = data.friendslist
    ? data.friendslist.friends.map((friend) => friend.friend_since)
    : [];
  return { friendIDs, friendsSince };
}

async function getPlayerProfiles(steamID) {
  const { friendIDs, friendsSince } = await getfriendIDs(steamID);
  const userArr = await getPlayerSummaries([steamID]);

  const user = userArr[0];
  const userProfile = {
    steamid: user.steamid,
    avatar: user.avatarfull,
    personaname: user.personaname,
    profileurl: user.profileurl,
    timecreated: user.timecreated ? new Date(user.timecreated * 1000) : null,
    loccountrycode: user.loccountrycode,
  };

  let friendsProfiles = [];
  if (friendIDs.length > 0) {
    friendsArr = await getPlayerSummaries(friendIDs);
    friendsProfiles = friendsArr.map((profile, i) => ({
      steamid: profile.steamid,
      avatar: profile.avatarfull,
      personaname: profile.personaname,
      profileurl: profile.profileurl,
      timecreated: profile.timecreated
        ? new Date(profile.timecreated * 1000)
        : null,
      loccountrycode: profile.loccountrycode,
      friendsSince: friendsSince[i] ? new Date(friendsSince[i] * 1000) : null,
    }));
  }
  return { friendsProfiles, userProfile };
}

async function getLowestGamePrice(appid) {
  // if (gamePriceCache.has(appid)) {
  //   console.log(`Cache HIT for AppID ${appid}: ${gamePriceCache.get(appid)}`);
  //   return gamePriceCache.get(appid);
  // }
    console.log(` Cache MISS for AppID ${appid} - Making API call`);
   try {
    const plainID = await priceLimit(() =>
      fetch(
        `https://api.isthereanydeal.com/games/lookup/v1?key=${process.env.ITAD_API_KEY}&appid=${appid}`
      )
    );

    if (!plainID.ok) {
      console.error(`Lookup API failed for AppID ${appid}: ${plainID.status}`);
      // gamePriceCache.set(appid, null);
      return null;
    }
    const plainData = await plainID.json();
    if (!plainData || !plainData.game || !plainData.game.id) {
      console.error(`No game found for AppID ${appid}`);
      // gamePriceCache.set(appid, null);
      return null;
    }
  const plainIDValue = [plainData.game.id];
  // console.log("Plain ID for AppID", appid, "is", plainIDValue);

  const response = await priceLimit(() =>
    fetch(
      `https://api.isthereanydeal.com/games/storelow/v2?key=${process.env.ITAD_API_KEY}&country=PT&shops=61`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      },
      body: JSON.stringify(plainIDValue),
    }
  ));
  console.log("Request body for lowest price:", JSON.stringify(plainIDValue));
  if (!response.ok || response.status !== 200) {
    // console.error(`API error for AppID ${appid}: ${response.status}`);
    // gamePriceCache.set(appid, null);
    return null;
  }
  const data = await response.json();
  // console.log("Lowest Price API response status:", response.status, "AppID:", appid);
  let price = null;
  if (
    Array.isArray(data) &&
    data.length > 0 &&
    data[0].lows &&
    data[0].lows.length > 0
  ) {
    price = data[0].lows[0].price.amountInt || null;
  }
  // gamePriceCache.set(appid, price);
  console.log(`💾 Cached price for AppID ${appid}: ${price}`);
  return price;
   } catch (error) {
    console.error(`Complete error in getLowestGamePrice for AppID ${appid}:`, error);
    // gamePriceCache.set(appid, null);
    return null;
  }
}

async function getGameGenres(appid) {
  const response = await fetch(
    `https://store.steampowered.com/api/appdetails?appids=${appid}&l=portuguese`
  );
  if (response.status !== 200) {
    return [];
  }
  const data = await response.json();
  return data[appid]?.data?.genres || [];
}

async function getGamePosters(appid) {
  try {
    const response = await posterLimit(() => fetch(`https://store.steampowered.com/api/appdetails?appids=${appid}`));
    const data = await response.json();

    if (data[appid]?.success) {
      return data[appid].data.header_image;
    }
    return null;
  } catch (error) {
    console.error("Error fetching Steam game poster:", error);
    return null;
  }
}
async function getOwnedGames(steamid) {
  if (typeof steamid !== "string") {
    console.log("Invalid steamid:", steamid);
  }
  console.log("Fetching games for steamid:", steamid);

  const url = `https://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=${API_KEY}&steamid=${steamid}&include_appinfo=true&format=json`;
  const response = await gameLimit(() => fetch(url));
  if (response === null || response.status !== 200) {
    return [];
  }
  console.log("Steam API status:", response.status, "URL:", url);
  const data = await response.json();
  return data.response.games || [];
}

async function getGameAchievements(appid) {
  const response = await fetch(
    `https://api.steampowered.com/ISteamUserStats/GetSchemaForGame/v0002/?appid=${appid}&key=${API_KEY}`
  );
  const data = await response.json();
  const achievements = data?.game?.availableGameStats?.achievements || [];
  return achievements;
}

async function getGameGlobalAchievements(appid) {
  const response = await fetch(
    `https://api.steampowered.com/ISteamUserStats/GetGlobalAchievementPercentagesForApp/v0002/?gameid=${appid}`
  );
  const data = await response.json();
  const globalAchievements = data?.achievementpercentages?.achievements || [];

  return globalAchievements;
}

async function getAchievements(steamid, appid) {
  const response = await fetch(
    `https://api.steampowered.com/ISteamUserStats/GetPlayerAchievements/v0001/?appid=${appid}&key=${API_KEY}&steamid=${steamid}`
  );
  const data = await response.json();
  return data.playerstats.achievements || [];
}

async function checkAchievements(steamid, appid) {
  const achievements = await getGameAchievements(appid);
  const playerAchievements = await getAchievements(steamid, appid);
  const globalAchievements = await getGameGlobalAchievements(appid);
  // console.log(globalAchievements);
  // console.log(achievements);
  // console.log(playerAchievements);

  const achievementStatus = achievements.map((ach) => {
    const playerAch = playerAchievements.find((pa) => pa.apiname === ach.name);
    const globalAch = globalAchievements.find((ga) => ga.name === ach.name);

    return {
      apiname: ach.name,
      achieved: playerAch ? !!playerAch.achieved : false,
      unlocktime:
        playerAch && playerAch.unlocktime
          ? new Date(playerAch.unlocktime * 1000)
          : null,
      globalPercentage: globalAch ? globalAch.percent : 0,
      icon: ach.icon,
      icongray: !ach.achieved ? ach.icongray : null,
    };
  });
  // console.log(achievementStatus);
  return achievementStatus;
}

module.exports = {
  getfriendIDs,
  getPlayerProfiles,
  getOwnedGames,
  checkAchievements,
  getGameGenres,
  getLowestGamePrice,
  getGamePosters,
};

// função de teste ******* testado, funciona *******
// (async () => {
//   try {
//     const result = await getfriendIDs("76561198067155799");
//     console.log(result);
//     const friends = await getPlayerProfiles("76561198067155799");

//     console.log(friends);
//   } catch (err) {
//     console.error("Error fetching friends:", err);
//   }
// })();

// função de teste ******* testado, funciona *******
// (async () => {
//   try {
//     const friends = await getfriendIDs("76561198067155799");
//     console.log(friends);
//   } catch (err) {
//     console.error("Error fetching friends:", err);
//   }
// })();

// (async () => {
//   try {
//     const result = await getOwnedGames("76561198067155799");
//     console.log(result);

//   } catch (err) {
//     console.error("Error fetching friends:", err);
//   }
// })();