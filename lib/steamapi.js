...
async function getOwnedGames(steamID) {
  const response = await fetch(
    `https://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=${API_KEY}&steamid=${steamID}&include_appinfo=true&format=json`
  );
  const data = await response.json();
  return data.response.games || [];
}

module.exports = { getfriendIDs, getPlayerProfiles, getOwnedGames };
