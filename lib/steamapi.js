...
const gameSchemaCache = new Map(); // cache for achievement schemas
const globalAchievementsCache = new Map(); // cache for global achievements
// const gamePriceCache = new Map(); // cache for game prices
...
//  console.log("Plain ID for AppID", appid, "is", plainIDValue);
  ...
  const response = await fetch(
    ...
    let price = null;
    if (Array.isArray(data) && data.length > 0 && data[0].lows && data[0].lows.length > 0) {
...
async function findCommonGames(steamids) {
...
async function preloadGameSchemas( appids) {
 const allAchievements = await Promise.all(
   appids.map(async (appid) => {
     return await getGameAchievements(appid);
   })
 );
 const allGlobalAchievements = await Promise.all(
   appids.map(async (appid) => {
     return await getGameGlobalAchievements(appid);
   })
 );
...
if (gameSchemaCache.has(appid)) {
    return gameSchemaCache.get(appid);
  }
    const response = await fetch(
    ...
    const data = await response.json();
     ...
     gameSchemaCache.set(appid, achievements);
     return achievements;
}

async function getGameGlobalAchievements(appid) {
    if (globalAchievementsCache.has(appid)) {
        return globalAchievementsCache.get(appid);
    }
    const response = await fetch(
    ...
    globalAchievementsCache.set(appid, globalAchievements);
...
module.exports = { getfriendIDs, getPlayerProfiles, getOwnedGames, checkAchievements, getGameGenres, getLowestGamePrice, findCommonGames, preloadGameSchemas };
